...
* Switch from emacs-pgtk to emacs-gtk3 for terminal client support :emacs:container:pgtk:gtk3:
:PROPERTIES:
:DATE: 2026-03-26
:END:

Emacs PGTK build has a known bug: =emacsclient -nw= / =-t= fails to create
terminal frames. The PGTK backend assumes a graphical display context and
doesn't properly handle tty frame creation.

** Impact
- =emacsclient -e= works fine (no frame needed, just eval)
- =emacsclient -c= works (GUI frame) when display is available
- =emacsclient -nw= / =-t= broken — the key use case for agents in terminal

** Decision
Switched from =emacs-pgtk-nativecomp= to =emacs-gtk3-nativecomp= in Containerfile.
PGTK is only needed for native Wayland rendering, which isn't used inside
the container. GTK3 handles both GUI (X11) and terminal frames.

** Also fixed
- Added =grep= package (GNU grep) — BusyBox grep doesn't support =--exclude-dir=
  flags used by the zshrc alias =my_grep_options=.
...
Works with PGTK too (=send-string-to-terminal= works despite being GUI).
...
- =emacs-pgtk-nativecomp= — native Wayland (broken terminal client)
- =emacs-gtk3-nativecomp= — GTK3/X11 (works for everything)
- =emacs-x11-nativecomp= — X11 only

** Refs
- Emacs debbugs — PGTK terminal frame creation issues
- Emacs 31 expected to fix PGTK terminal frames
