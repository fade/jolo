...
- =rusty_v8= has no musl prebuilts (any version); =codex-acp= pinned to =678a99ec= to avoid v8 build failure :alpine:musl:codex:
...
- Custom skills use =jolo:= namespace prefix to avoid collision with third-party skill packs (e.g. superpowers) :skills:workflow:
